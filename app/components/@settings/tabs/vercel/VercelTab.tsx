...
export default function VercelTab() {
  ...
  const autoDeploy = useStore(vercelAutoDeploy);
    ...
    <div className="space-y-6">
      ...
      >
        <div className="p-6 flex items-start justify-between gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <span className="i-ph:rocket-launch w-4 h-4 text-bolt-elements-item-contentAccent" />
              <h3 className="text-sm font-medium text-bolt-elements-textPrimary">Deploy when generation finishes</h3>
            </div>
            <p className="text-xs text-bolt-elements-textSecondary leading-relaxed">
              As soon as the generated app is running, bolt builds it and ships it to Vercel - no click needed. Runs
              once per chat, and only for projects generated in the current page view, so reloading a chat does not
              redeploy it.{' '}
              {connection.token
                ? 'Deploys use the token held by the browser.'
                : 'Deploys are signed with VERCEL_TOKEN on the server.'}
            </p>
          </div>
          <Switch checked={autoDeploy} onCheckedChange={(checked) => setVercelAutoDeploy(checked)} />
        </div>
      </motion.div>
