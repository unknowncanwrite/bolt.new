...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  const token = resolveVercelToken(url.searchParams.get('token'), context?.cloudflare?.env as any);

  if (!projectId || !token) {
    return json({ error: projectId ? VERCEL_TOKEN_MISSING_MESSAGE : 'Missing projectId' }, { status: 400 });
  }
...
export async function action({ request, context }: ActionFunctionArgs) {
  try {
    ...
    /*
     * The token may come from the browser (manual connection in Settings) or from the
     * server environment (`VERCEL_TOKEN` / `VITE_VERCEL_ACCESS_TOKEN`), which keeps the
     * secret out of the client bundle and localStorage.
     */
    const token = resolveVercelToken(clientToken, context?.cloudflare?.env as any);
